...
- **Editing the canvas node `<textarea>` in E2E**: `startEdit` in `Canvas.tsx` focuses + selects the editor via a deferred `setTimeout`. Tests must `await expect(ta).toBeFocused()` before typing and type with `pressSequentially` (not `fill`) — a one-shot `fill()` can race the deferred `select()` and desync React's controlled value tracker, intermittently wiping typed text. Such specs pass in isolation but fail under full-suite parallel load, so always run the full `npm run test:e2e` (and `--repeat-each` to stress) before trusting them.
...
- **Canvas** (`src/components/Canvas.tsx`): SVG-based canvas handling pan (drag background, or scroll/touchpad — scroll/two-finger swipe PANS the view and no longer zooms), zoom (via the on-canvas +/− buttons or keyboard shortcuts: `+`/`=` zoom in, `-`/`_` zoom out, `0` reset to 100%; touchscreen two-finger pinch-to-zoom is still supported), node drag, inline editing (HTML `<textarea>` overlaid on SVG), and selection. View transform is `translate(tx,ty) scale(s)` on a top-level `<g>`. Edges are cubic Bézier paths. Also handles node collapse/expand, reparent mode, and custom cross-link creation. Inline-edit keys: Enter finishes editing; Shift+Enter finishes and adds a sibling; Alt+Enter (Opt+Enter on Mac) inserts an explicit line break (`\n`) at the caret. Explicit `\n` is honored by `wrapText`/`measureNode`, so line breaks render on canvas, affect node sizing, and carry through to SVG export.
